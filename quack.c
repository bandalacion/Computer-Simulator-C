...
static uint8_t io_read8(uint16_t addr) {
    switch (addr) {
    case IO_TICK: return (uint8_t)(tick_counter & 0xFF);
    case IO_STATUS: return key_available ? 1:0;
    case IO_KEY: if(key_available) {
        uint8_t k = last_key;
        key_available = 0;
        last_key = 0;
        return k;
...
static void io_write8(uint16_t addr, uint8_t v) {
    switch(addr) {
        case IO_PUTCHAR: putchar((int)v);
        fflush(stdout);
        break;
    case IO_CLEAR: printf("\033[2J\033[H");
    fflush(stdout);
    break;
    default:
    break;
...
static uint8_t routed_read8(uint16_t addr) {
    if (addr >= IO_START && addr <= IO_END)
        return io_read8(addr);
    return mem_read8(addr);
}

static void routed_write8(uint16_t addr, uint8_t v) {
    if (addr >= IO_START && addr <= IO_END)
        io_write8(addr, v);
    else
        mem_write8(addr, v);
...
static void cpu_step(cpu_t *cpu, int debug) {
    ...
    */
    ...
    case OP_RMMOVW:
        check_reg(in.ra);
        imm16 = u16_from_le(in.b2, in.b3);
        mem_write16(imm16, cpu->r[in.ra]);
        cpu -> pc += 4;
        break;
    case OP_MRMOVB:
        check_reg(in.ra);
        imm16 = u16_from_le(in.b2, in.b3);
        cpu -> r[in.ra] = (uint16_t)routed_read8(imm16);
        cpu -> zf = (cpu -> r[in.ra] ==0);
        cpu -> pc += 4;
        break;
    case OP_RMMOVB:
        check_reg(in.ra);
        imm16 = u16_from_le(in.b2, in.b3);
        routed_write8(imm16, (uint8_t)(cpu -> r[in.ra] & 0xFF));
        cpu -> pc += 4;
        break;
    case OP_RMMOVBR:
        check_reg(in.ra);
        rb = in.b2;
        check_reg(rb);
        routed_write8(cpu->r[rb], (uint8_t)(cpu-> r[in.ra] & 0xFF));
        cpu-> pc += 4;
        break;
    case OP_MRMOVBR:
        check_reg(in.ra);
        rb = in.b2;
        check_reg(rb);
        cpu -> r[in.ra] = (uint16_t)routed_read8(cpu->r[rb]);
        cpu -> zf = (cpu->r[in.ra] == 0);
        cpu -> pc +=4;
        break;
    case OP_OUTC:
        check_reg(in.ra);
        putchar((int)(cpu-> r[in.ra] & 0xFF));
        fflush(stdout);
        cpu -> pc += 4;
        break;
    /*
